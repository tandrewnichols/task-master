...
module.exports = function(grunt, opts) {
  ...
  ['tasks', 'include', 'exclude'].forEach(function(key) {
    options[key] = Array.isArray(options[key]) ? options[key] : [options[key]];
  });

  // Try to load package.json
  var root = __dirname.indexOf('node_modules') > 0 ? __dirname.split('node_modules')[0] : process.cwd();
  ...
  options.tasks.forEach(function(dir) {
    config = fm.generate(root + '/' + dir, { memo: config, reducer: function(options, manifest, file) {
