...
exports.getAll = function(patterns, root, dirs) {
  ...
  var files = _.reduce(dirs, function(manifest, dir) {
    return _.reduce(patterns, function(inner, pattern) {
  ...
  return _.reduce(files, function(manifest, file) {
    return _.extend({}, manifest, exports.get(file));
  }, {});
};
...
exports.load = function(name, value, root, dirs) {
  ...
  if (_.isArray(value)) {
    // If it's array, get the matching files and merge the contents 
    value = exports.getAll(value, root, dirs); 
  } else if (_.isEmpty(value)) {
    // If it doesn't exist, look for the canoncial file
    value = exports.getAll('_taskmaster.' + name + '.{js,coffee,json,yml}', root, dirs);
  }
