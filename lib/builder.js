  ...
  _.each(options.tasks, function(dir) {
    config = fm.generate(root + '/' + dir, {
      ...
      reduce: function(manifest, file) {
        ...
        var contents = loader.get(file.abs(), grunt, manifest);
        // Get any config overrides
        var name = file.name();
        var overrides = loader.load('override.' + name, null, root, [dir]);

        if (contents) {
          // Assign task configuration, recursively, to main config object
          exports.assignManifest(name, contents, manifest);
        }
        
        // Override configuration if provided
        var overrideConfig = {};
        if (!_.isEmpty(overrides)) {
          overrideConfig[name] = overrides;
        }
